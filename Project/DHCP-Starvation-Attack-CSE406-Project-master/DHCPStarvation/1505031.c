...
int makeDHCPDiscoverPacket(int sock){
   ...
   fprintf(fptr,"@Discover#68#67#-#%s",
		inet_ntoa(socketAddressBroadcast.sin_addr));
			fprintf(fptr,"#%s",inet_ntoa(discoverPacket.ciaddr));
fprintf(fptr,"#%s",inet_ntoa(discoverPacket.yiaddr));
fprintf(fptr,"#%s",inet_ntoa(discoverPacket.siaddr));
fprintf(fptr,"#%s#",inet_ntoa(discoverPacket.giaddr));

		  for(int x=0;x<ETHERNET_HARDWARE_ADDRESS_LENGTH;x++)
                                fprintf(fptr,"%02X:",(unsigned char)discoverPacket.chaddr[x]);
 fprintf(fptr,"#(0x%X)\n",ntohl(discoverPacket.xid));
...
if(DEBUG){
        ...
        printf("DHCPREQUEST XID: %lu (0x%X)\n",(unsigned long) ntohl(requestPacket.xid),ntohl(requestPacket.xid));
        printf("DHCPREQUEST ciaddr:  %s\n",inet_ntoa(requestPacket.ciaddr));
        printf("DHCPREQUEST yiaddr:  %s\n",inet_ntoa(requestPacket.yiaddr));
        printf("DHCPREQUEST siaddr:  %s\n",inet_ntoa(requestPacket.siaddr));
        printf("DHCPREQUEST giaddr:  %s\n",inet_ntoa(requestPacket.giaddr));
        }
    ...
    long    tv_usec;        // microseconds 
        ...
        if(!FD_ISSET(sock,&readfds)){
                ...
                if(receivedData==-1){
                        ...
                        if (DEBUG) {
                                printf("receivePacket() result: %d\n",receivedData);
                                printf("receivePacket() source: %s\n",inet_ntoa(sourceAddress.sin_addr));
                        }
...
int getDHCPOfferPacket(int sock){
        ...
        while(1){
		   ...
		   fprintf(fptr,"@Offer#67#68#%s#255.255.255.255",
		inet_ntoa(source.sin_addr));
			fprintf(fptr,"#%s",inet_ntoa(offer_packet.ciaddr));
fprintf(fptr,"#%s",inet_ntoa(offer_packet.yiaddr));
fprintf(fptr,"#%s",inet_ntoa(offer_packet.siaddr));
fprintf(fptr,"#%s#",inet_ntoa(offer_packet.giaddr));


		  for(x=0;x<ETHERNET_HARDWARE_ADDRESS_LENGTH;x++)
                                fprintf(fptr,"%02X:",(unsigned char)offer_packet.chaddr[x]);
...
{
       strncpy(myInterface,argv[1],sizeof(myInterface)-1);
            myInterface[sizeof(myInterface)-1]='\x0';
FILE *fptr;
